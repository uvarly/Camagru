...
window.onload = function() {
    ...
    for (i = 0; i < images.length; i++) {
        images[i].onclick = img_list(images[i].currentSrc);
    }
...
function readURL(input) {
    let images = document.getElementsByTagName('img');
    for (i = 0; i < images.length; i++) {
        images[i].style.display = 'inline-flex';
    }
    img = new Image;
...
function submit() {
    ...
    for (i = 0; i < stickers.length; i++) {
        ...
        input.setAttribute('value', str[5] + ';' + stickers[i].X + ';' + stickers[i].Y);
        form.appendChild(input);
    }
    if (source === 2)
        document.getElementById('submit').click();
    else if (source === 1) {
        document.getElementById('upload_form').setAttribute('action', '/camera/upload_base/')
